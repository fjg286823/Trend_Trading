...
- 本地 CSV 数据导入
- 输入股票/ETF代码查看指定标的 K 线
- 批量输入股票池并按指定区间运行策略回测
- 单独查看每只股票的买卖点、资金曲线和交易日志
- 基线趋势策略回测
- 虚拟资金模拟撮合
- K 线图、均线、买入/加仓/卖出点展示
- 交易日志与资金曲线
...
```text
510300, 159915, 600519, 000001
```

在线行情加载顺序：

1. 东方财富直连，禁用系统代理，优先获取完整区间。
2. 腾讯行情直连，作为备用源；该接口通常最多返回约 640 根日 K。
3. AKShare，作为最后回退。

如果本机代理、VPN 或防火墙导致在线数据失败，可以先关闭代理后重试，或使用 CSV/示例数据。
